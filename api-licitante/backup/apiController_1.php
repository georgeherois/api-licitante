...
class apiController extends controller {
	...
	private function fetchDataFromApiSegundo($cnpj, $sequencialCompra, $tipo){
		$dados = [];
		...
		foreach($dados as $item){
			if(is_array($item) === true && $item['materialOuServico'] === $tipo){
				$material[] = $item;
			}
		}
			...
			if (!empty($firstApiResponse['data'])){
				foreach ($firstApiResponse['data'] as $contrato) {
					...
					if (!empty($cnpj) && !empty($sequencialCompra)){
						...
						$secondApiResponse = $this->fetchDataFromApiSegundo($cnpj, $sequencialCompra, $tipo);
							...
							// }
            							...
            							if($dataEncerramentoProposta === $dataFinal && !empty($secondApiResponse)){
